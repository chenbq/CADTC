...
# simplified version for atoc

class ATOCNet_sim(AgentBaseNet):
    """
    ...
    # comm_top记录的是通信拓扑，如果agent_i与任意agent_j没有联系，那么comm_top(i,j)=0
    # 否则，comm_top(i,j)=1
    ...
    def get_comm_dQ_result(self, obs, nearby_agents, comm_top=None, mode='initial', com_flag=False, explore_flag=True):
        ...
        if mode == 'initial':
            ...
            comm_top = torch.from_numpy(np.array(nearby_agents)).to('cuda') * head_flag
            comm_top = comm_top.cpu()
            already_com_set = []
        ...
        for i in range(self.nagents):
            ...
            thought_after[idx_mat_t] = thought_after_i.reshape(-1)
            comm_counts += comm_top[i].detach().numpy()
