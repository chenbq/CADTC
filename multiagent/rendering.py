...
class Text(Geom):
    def __init__(self, text='label', position=(0.2, 0.2)):
        Geom.__init__(self)
        self.position = position
        self.text = text
        self.label = pyglet.text.Label(text, font_size=50,
                                  x=self.position[0], y=self.position[1], anchor_x='left', anchor_y='top',
                                  color=(255, 255, 255, 255),)
    def render1(self):
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        glEnable(GL_BLEND)
        # glBegin(GL_TEXTURE_2D)  # draw texture
        # self.label.draw()
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12,self.text)
        # glEnd()
