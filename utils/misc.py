...
def soft_update_lst(target, source, tau):
    """
    Perform DDPG soft update (move target params toward source based on weight
    factor tau)
    Inputs:
    ...
    for i in range(len(target)):
        for target_param, param in zip(target[i].parameters(), source[i].parameters()):
            target_param.data.copy_(target_param.data * (1.0 - tau) + param.data * tau)
...
def hard_update_lst(target_lst, source_lst):
    """
    Copy network parameters from source to target
    Inputs:
        target (torch.nn.Module): Net to copy parameters to
    ...
    for i in range(len(target_lst)):
        for target_param, param in zip(target_lst[i].parameters(), source_lst[i].parameters()):
            target_param.data.copy_(param.data)
...
def gumbel_softmax_sample(logits, temperature):
    ...
    samples= sample_gumbel(logits.shape, tens_type=type(logits.data)).to(logits.device)
    y = logits +samples
